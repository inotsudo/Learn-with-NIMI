...
export default function AchievementCenterCard({ achievements, maxLevel, childName }: Props) {
  ...
  return (
    <div className="bg-white border-2 border-purple-200 rounded-2xl shadow-md p-4">
      <p className="font-black text-gray-800 mb-3">{t("achievementCenterTitle")}</p>

      <div className="grid grid-cols-3 gap-2 mb-3">
        {tiles.map(tile => (
          <div key={tile.labelKey} className="bg-gray-50 rounded-xl p-3 text-center">
            <p className="text-2xl">{tile.emoji}</p>
            <p className="font-black text-gray-800 text-sm mt-1">{earnedCount(tile.items)}/{tile.items.length}</p>
            <p className="text-gray-400 text-xs mt-0.5 leading-tight">{t(tile.labelKey)}</p>
          </div>
        ))}
      </div>
      ...
      <Link href="/certificates" className="block text-center text-purple-600 font-black text-sm hover:underline">
