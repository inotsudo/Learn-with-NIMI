          ...
          return (
            <div key={cat}>
              <div className="flex items-center gap-2 mb-3">
                ...
                <div>
                  <p className="font-black text-mbase leading-tight" style={{ color: "#082b78" }}>{t(`filter${cat.charAt(0).toUpperCase() + cat.slice(1)}`)}</p>
                  <p className="text-2xs" style={{ color: "#7791b3" }}>{t(meta.descKey)}</p>
                </div>
              </div>
      ...
      >
        {items.length === 0 ? (
          <div className="flex flex-col items-center text-center gap-3 py-16">
            ...
            <p className="font-black" style={{ color: "#082b78" }}>{t("noToysTitle")}</p>
            <p className="text-sm" style={{ color: "#7791b3" }}>{t("noToysBody")}</p>
          </div>
        ) : (
          <GridRow items={items} balance={balance} ownedIds={ownedIds} purchaseCounts={purchaseCounts}
            cosmetics={cosmetics} purchasingId={purchasingId}
