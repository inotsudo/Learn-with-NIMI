...
export default function MyProfile({ childName, avatar, categoriesMastered }: Props) {
  return (
    <div className="bg-white/10 backdrop-blur border-2 border-white/15 rounded-2xl shadow-md p-4 text-center flex flex-col h-full">
      <h3 className="font-black theme-text text-[12px] uppercase mb-3 tracking-widest">MY PROFILE</h3>
      <div className="relative inline-block mb-2 mx-auto">
        {avatar && !avatar.startsWith("http") ? (
          <div className="w-20 h-20 rounded-full border-4 theme-border-strong shadow-lg bg-gradient-to-br from-purple-100 to-indigo-100 flex items-center justify-center text-4xl select-none">
            {avatar}
          </div>
        ) : (
          <img
            src={avatar ?? "/default-avatar.png"} alt={childName}
            className="w-20 h-20 rounded-full object-cover border-4 theme-border-strong shadow-lg"
      ...
      <div className="flex items-center justify-center gap-1 mt-1 mb-0.5">
        ...
        <Edit className="w-3.5 h-3.5 theme-text-muted" />
      </div>
      <p className="text-[11px] font-bold text-yellow-300 mb-3">⭐ SUPER STAR ★</p>
      <div className="text-left mb-3">
        <p className="text-[10px] theme-text-muted font-semibold mb-1">Categories Mastered</p>
        ...
        <p className="text-[10px] theme-text-muted text-right">{categoriesMastered}/8</p>
      </div>
      <div className="mt-auto">
        <Link href="/user-profile">
          <Button className="w-full theme-accent hover:theme-accent text-white text-[10px] font-black rounded-full h-8 tracking-wide">
