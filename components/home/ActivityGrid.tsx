        ...
        return (
          <Link key={activity.number} href={activity.href}>
            <motion.div
              ...
              whileTap={{ scale: 0.97 }}
              className={`relative bg-white border-2 ${activity.borderColor} rounded-2xl shadow-sm hover:shadow-xl transition-all cursor-pointer flex flex-col items-center pt-3 pb-3 px-2 h-full`}
            >
              {/* Number badge */}
              ...
              {/* Stars pill */}
              <div className="absolute top-2 right-2 bg-yellow-50 border border-yellow-200 rounded-full px-1.5 py-0.5 text-[9px] font-black text-yellow-600 flex items-center gap-0.5">
                ⭐ {stars}
              </div>
              ...
              <div className="relative w-16 h-16 mt-4 mb-2 flex items-center justify-center">
                ...
                />
                <div className="absolute -bottom-1 -right-1 bg-white rounded-full w-6 h-6 flex items-center justify-center text-sm shadow border border-gray-100 leading-none">
                  {activity.emoji}
                </div>
              </div>

              <p className="font-black text-gray-800 text-[11px] uppercase text-center leading-tight">
                {t(activity.titleKey)}
              </p>
              <p className="text-gray-500 text-[9px] text-center leading-snug mt-0.5 min-h-[24px]">
                {t(activity.subtitleKey)}
              </p>

              {/* Progress dot */}
              <div className={`w-2.5 h-2.5 rounded-full mt-1 ${done ? "bg-green-500" : "bg-gray-200"}`} />
            </motion.div>
          </Link>
        );
      })}
