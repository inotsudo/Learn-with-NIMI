            ...
            <table className="w-full text-left">
              ...
              <tbody>
                {pageRows.length === 0 ? (
                  ...
                  <tr key={c.id} className="border-b border-gray-50 hover:bg-indigo-50/20 transition">
                    <td className="px-5 py-3.5">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-full bg-indigo-100 flex items-center justify-center text-indigo-600 text-[16px] font-black shrink-0 border border-indigo-200">
          ...
          <div className="flex items-center gap-1">
            ...
            {Array.from({ length: Math.min(totalPages, 5) }).map((_, i) => (
              <button key={i + 1} onClick={() => setPage(i + 1)}
                className={`w-8 h-8 rounded-lg text-[12px] font-bold transition ${i + 1 === pageClamped ? 'bg-indigo-600 text-white' : 'border border-gray-200 text-gray-500 hover:bg-gray-50'}`}>
                {i + 1}
              </button>
            ))}
